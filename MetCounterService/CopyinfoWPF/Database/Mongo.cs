    ...
    static class MongoTB
    {
        ...
        public async static Task InitializeAsync()
        {
            await Task.Run(() =>
           {
               ...
               MongoCredential credentials = MongoCredential.CreateCredential(databaseName, login, password);
        ...
        private static ObjectId SaveMachineRecord_Deleted(MachineRecord r)
        {
            if (r != null)
            {
                ...
                Log.LogMessage("SaveMachineRecord_Deleted: " + res.UpdatedExisting.ToString());
        ...
        private static ObjectId SaveFullHTMLSerial_Deleted(HTMLSerial r)
        {
            if (r != null)
            {
                ...
                var obj = r;
                WriteConcernResult res = col.Save(obj);
                Log.LogMessage("SaveFullHTMLSerial_Deleted: " + res.UpdatedExisting.ToString());
                return (ObjectId)obj.id;
            }
            else
            {
        ...
        private static ObjectId SaveFullHTMLCounter_Deleted(HTMLCounter r)
        {
            if (r != null)
            {
                ...
                var obj = r;
                WriteConcernResult res = col.Save(obj);
                Log.LogMessage("SaveFullHTMLCounter_Deleted: " + res.UpdatedExisting.ToString());
                return (ObjectId)obj.id;
            }
            else
            {
